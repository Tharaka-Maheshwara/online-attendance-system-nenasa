...
async function testAttendanceFiltering() {
  try {
    ...
    const ourStudents = allStudents.filter(s => createdStudentIds.includes(s.id));
    
    console.log("Our test students:");
    ourStudents.forEach(student => {
      const subjects = [student.sub_1, student.sub_2, student.sub_3, student.sub_4]
        .filter(Boolean);
    ...
    const mathClass = classes.find(c => c.name.toLowerCase().includes('math') || c.name === 'Mathematics');
    if (mathClass) {
      console.log(`\nTesting filter for "${mathClass.name}" class:`);
      const mathStudents = allStudents.filter(student => {
        const studentSubjects = [
          student.sub_1, student.sub_2, student.sub_3, student.sub_4
        ].filter(Boolean);
        
        return studentSubjects.some(subject => 
          subject.toLowerCase() === mathClass.name.toLowerCase()
        );
      });
    ...
    if (itClass) {
      ...
      const itStudents = allStudents.filter(student => {
        ...
        return studentSubjects.some(subject => 
          subject.toLowerCase() === itClass.name.toLowerCase()
        );
      });
    ...
    const otherClasses = classes.filter(c => 
      !['Mathematics', 'IT', 'Physics', 'Chemistry', 'Biology'].includes(c.name)
    );
    ...
    if (otherClasses.length > 0) {
      ...
      const noStudents = allStudents.filter(student => {
        ...
        return studentSubjects.some(subject => 
          subject.toLowerCase() === testClass.name.toLowerCase()
        );
      });
      
      console.log(`Testing "${testClass.name}" class: ${noStudents.length} students (should be 0)`);
    }

    console.log("\n✅ Attendance filtering test completed!");
    console.log("\n📝 Summary:");
    console.log("- Students are now filtered based on their enrolled subjects");
    console.log("- Only students with matching subjects appear in attendance lists");
