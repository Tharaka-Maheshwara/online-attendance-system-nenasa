...
export class AttendanceController {
		...
		try {
			...
			if (attendanceData.attendance && Array.isArray(attendanceData.attendance)) {
				...
				for (const record of attendanceData.attendance) {
					...
					const savedAttendance = await this.attendanceService.create(attendance);
	...
	async findByClassAndDate(@Param('classId') classId: number, @Param('date') date: string): Promise<Attendance[]> {
