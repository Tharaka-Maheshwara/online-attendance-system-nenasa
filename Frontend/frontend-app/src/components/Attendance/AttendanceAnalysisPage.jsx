...
const AttendanceAnalysisPage = () => {
  ...
  const fetchGrades = async () => {
    try {
      ...
      setError(
        "Failed to load grades. Please check if the backend is running."
      );
      setGrades([]);
    }
  };
  ...
  const fetchSubjects = async () => {
    try {
      const response = await fetch(
        `${API_BASE}/analysis/subjects/${selectedGrade}`
      );
  ...
  const fetchClassesByGrade = async () => {
    try {
      const response = await fetch(
        `${API_BASE}/classes/by-grade/${selectedGrade}`
      );
  ...
  const fetchAnalysis = async () => {
    ...
    setError("");
    try {
      const url = `${API_BASE}/analysis/comprehensive/${selectedGrade}/${encodeURIComponent(
        selectedSubject
      )}?timeRange=${timeRange}`;
    ...
    analysisData?.students?.length > 0
      ? {
          ...
          datasets: [
            {
              data: [
                analysisData.students.reduce(
                  (sum, student) => sum + student.attendanceStats.presentCount,
                  0
                ),
                analysisData.students.reduce(
                  (sum, student) => sum + student.attendanceStats.absentCount,
                  0
                ),
                analysisData.students.reduce(
                  (sum, student) => sum + student.attendanceStats.lateCount,
                  0
                ),
              ],
              backgroundColor: [
  ...
  return (
    <div className="container mx-auto p-6 bg-gray-50 min-h-screen">
      <h1 className="text-4xl font-bold mb-8 text-gray-800">
        Student Attendance History Analysis
      </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <div className="bg-gradient-to-r from-blue-500 to-blue-600 p-6 rounded-lg text-white">
              ...
              <p className="text-3xl font-bold">
                {analysisData.summary?.totalStudents || 0}
              </p>
            </div>
            <div className="bg-gradient-to-r from-green-500 to-green-600 p-6 rounded-lg text-white">
              ...
              <p className="text-3xl font-bold">
                {analysisData.summary?.overallAttendanceRate || 0}%
              </p>
            </div>
            <div className="bg-gradient-to-r from-purple-500 to-purple-600 p-6 rounded-lg text-white">
              ...
              <p className="text-3xl font-bold">
                {analysisData.summary?.totalClasses || 0}
              </p>
            </div>
            <div className="bg-gradient-to-r from-yellow-500 to-yellow-600 p-6 rounded-lg text-white">
              ...
              <p className="text-3xl font-bold">
                Rs. {analysisData.summary?.classInfo?.monthlyFees || 0}
              </p>
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="text-center p-4 bg-green-50 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    {analysisData.paymentSummary.paidStudents} /{" "}
                    {analysisData.paymentSummary.totalStudents}
                  </p>
                </div>
                <div className="text-center p-4 bg-yellow-50 rounded-lg">
                  <p className="text-sm text-gray-600 mb-1">Pending Payments</p>
                  <p className="text-2xl font-bold text-yellow-600">
                    {analysisData.paymentSummary.pendingStudents}
                  </p>
                </div>
                <div className="text-center p-4 bg-red-50 rounded-lg">
                  <p className="text-sm text-gray-600 mb-1">Overdue Payments</p>
                  <p className="text-2xl font-bold text-red-600">
                    {analysisData.paymentSummary.overdueStudents}
                  </p>
                </div>
                <div className="text-center p-4 bg-blue-50 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-blue-600">
                    Rs. {analysisData.paymentSummary.collectedRevenue}
                  </p>
                </div>
              </div>
            </div>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {analysisData.students.map((student) => (
                      <tr key={student.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`text-sm font-bold ${getAttendanceRateColor(
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {student.attendanceStats?.presentCount || 0} /{" "}
      ...
      {!loading &&
        ...
        !error && (
          <div className="bg-yellow-50 border border-yellow-200 text-yellow-700 px-4 py-3 rounded mb-6">
            <p>
              No attendance data found for Grade {selectedGrade} -{" "}
              {selectedSubject}. Please ensure there are students registered and
