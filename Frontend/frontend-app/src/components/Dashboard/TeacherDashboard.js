...
const TeacherDashboard = () => {
    ...
    const fetchTeacherData = async () => {
      ...
      try {
        ...
        const teacherResponse = await fetch(
          `http://localhost:8000/teacher/by-email/${encodeURIComponent(
            userEmail
          )}`
        );
        ...
        const allClassesResponse = await fetch(`http://localhost:8000/class`);
        ...
        const teacherClasses = allClassesData.filter(
          (cls) =>
            cls.teacherName === teacherName || cls.teacherId === teacherId
        );
  ...
  const groupedClasses =
    activeTab === "all" ? groupClassesByDay(allClasses) : null;
        ...
        <div className="teacher-info-header">
          <div className="teacher-avatar">
            ...
            <div
              ...
              style={{ display: teacherInfo.profileImage ? "none" : "flex" }}
            >
              ...
              <div className="empty-state">
                ...
                {allClasses.length > 0 && (
                  <p style={{ marginTop: "10px", color: "#667eea" }}>
                    You have {allClasses.length} total{" "}
                    {allClasses.length === 1 ? "class" : "classes"}. Check the
                    "All My Classes" tab to see your full schedule.
                  </p>
                )}
              </div>
            )}
                    ...
                    classes.length > 0 && (
                      <div className="day-section" key={day}>
                        <div
                          ...
                          style={{ borderLeftColor: getDayColor(day) }}
                        >
                          <h3>{day}</h3>
              ...
              <div className="empty-state">
                ...
                {teacherInfo && (
                  ...
                  >
                    ...
                    >
                      Classes should have{" "}
                      <strong>teacherName = "{teacherInfo.name}"</strong> in the
                      database.
                    </p>
                  </div>
                )}
