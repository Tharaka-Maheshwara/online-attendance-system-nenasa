  ...
  const fetchAttendanceSummary = useCallback(async () => {
    ...
    try {
      ...
      currentMonthRecords.forEach(record => {
        const classKey = `${record.classInfo?.subject || 'Unknown'}_${record.classInfo?.grade || 'N/A'}`;
      ...
      Object.keys(attendanceByClass).forEach(classKey => {
        ...
        classData.attendanceRate = classData.totalClasses > 0 
          ? Math.round((classData.presentCount / classData.totalClasses) * 100)
          : 0;
        
        // Sort recent records by date (most recent first)
        classData.recentRecords.sort((a, b) => new Date(b.date) - new Date(a.date));
        ...
        const allRecordsForClass = attendanceRecords.filter(record => {
          const recordClassKey = `${record.classInfo?.subject || 'Unknown'}_${record.classInfo?.grade || 'N/A'}`;
          return recordClassKey === classKey;
        });
        classData.allRecords = allRecordsForClass.sort((a, b) => new Date(b.date) - new Date(a.date));
      });
    ...
    <div className="student-dashboard">
      <div className="dashboard-grid">
        ...
        <div className="personal-stats">
          <h2>My Attendance Summary - {new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</h2>
          <div className="stats-cards">
            <div className="stat-card primary">
              <div className="stat-icon">📊</div>
              <div className="stat-info">
                <h3>This Month's Attendance</h3>
                <p className="stat-percentage">{overallStats.attendanceRate}%</p>
                <span className="stat-detail">
                  {overallStats.attendanceRate >= 90 ? 'Excellent standing' :
                   overallStats.attendanceRate >= 75 ? 'Good standing' :
                   overallStats.attendanceRate >= 60 ? 'Average standing' : 'Needs improvement'}
                </span>
              </div>
            </div>
        ...
        <div className="attendance-by-class">
          <h2>My Attendance by Class - {new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</h2>
          <div className="attendance-content">
            {attendanceLoading ? (
                  ...
                  <div key={classKey} className="attendance-class-card">
                    <div className="attendance-card-header">
                      <div className="subject-info">
                        ...
                        <span className="grade-indicator">Grade {classData.grade}</span>
                      </div>
                    ...
                    <div className="attendance-card-content">
                      ...
                      <div className="attendance-stats">
                        <div className="month-indicator">
                          <span className="month-label">📅 {new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })} Statistics</span>
                        </div>
                        <div className="stat-row">
                          <div className="stat-item present">
                            <span className="stat-label">Present</span>
                            <span className="stat-value">{classData.presentCount}</span>
                          </div>
                          <div className="stat-item absent">
                            <span className="stat-label">Absent</span>
                            <span className="stat-value">{classData.absentCount}</span>
                            <span className="stat-percentage">
                              {classData.totalClasses > 0 ? 
                                Math.round((classData.absentCount / classData.totalClasses) * 100) : 0}%
                            </span>
                          </div>
                          <div className="stat-item late">
                            ...
                            <span className="stat-percentage">
                              {classData.totalClasses > 0 ? 
                                Math.round((classData.lateCount / classData.totalClasses) * 100) : 0}%
                            </span>
                          </div>
                          <div className="stat-item total">
                            <span className="stat-label">Total</span>
                            <span className="stat-value">{classData.totalClasses}</span>
                      ...
                      <div className="attendance-progress">
                        <div className="progress-label">
                          ...
                          <span className="progress-text">{classData.attendanceRate}%</span>
                        </div>
                        ...
                        {classData.recentRecords.length > 5 && (
                          <div className="view-more-container">
                            <button 
                              ...
                              onClick={() => {
                                const allRecords = classData.allRecords || classData.recentRecords;
                                alert(`Full attendance history for ${classData.subject}:\n\n${
                            ...
                            >
                              View Full History ({(classData.allRecords || classData.recentRecords).length} records)
                            </button>
                          </div>
                        )}
                      </div>
              ...
              <div className="no-attendance-message">
                <p>📊 No attendance records found for {new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}!</p>
                <span>Attendance records for this month will appear here once you start attending classes.</span>
              </div>
            )}
          </div>
        </div>
